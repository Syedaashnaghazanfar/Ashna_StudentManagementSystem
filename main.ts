...
// USING METHODS
    ...
    pay_fees(fees : number){
        this.balance -= fees as number;
...
//method for adding students
      add_students(NameOfStudent: string){
...
//method for enrolling students
      enrolled_students(studentID : number | undefined, course : string){
...
//method for viewing student balance
       viewing_students(studentID : number | undefined){
        let foundStudents = this.students.find(std => std.id === studentID)
        if(foundStudents){
 ...
 //method for paying students fees
        paying_fees(studentID : number | undefined, fees : number|undefined){
            let foundStudents = this.students.find(std => std.id === studentID)
            if(foundStudents){
                foundStudents.pay_fees(fees as number);
            }
            else{
                console.log(chalk.red(`Cannot find ${studentID},try again!`))
            }
 ...
 //method for showing student status
        show_status(studentID : number|undefined){
           let foundStudents = this.students.find(std => std.id === studentID)
           if(foundStudents){
